class login {
  ...
  typeInUsernameInputField(username) {
    cy.get(this.userNameInputField).should('be.visible');
    cy.get(this.userNameInputField).clear();
    cy.get(this.userNameInputField).type(username);
    cy.get(this.userNameInputField).should('have.value', username);
  }

  typePassword(password) {
    cy.get(this.passwordInputField).should('be.visible');
    cy.get(this.passwordInputField).clear();
    cy.get(this.passwordInputField).type(password);
    cy.get(this.passwordInputField).should('have.value', password);
  }
    ...
    cy.get(this.logoutButton)
      ...
      .then((buttonName) => {
        if (buttonName.trim() === 'LogOut') {
          ...
          cy.get(this.shippingAndWarrantySectionTitle).should(
            'have.text',
            'Shipping & Warranty'
          );
          cy.get(this.shippingDetailText).should(
            'have.text',
            'Ships in 2 weeks'
          );
          cy.get(this.warrantyDetailText).should(
            'have.text',
            '1 year warranty'
          );
